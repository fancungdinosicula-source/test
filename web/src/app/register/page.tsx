...
export default function RegisterPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col justify-center bg-gradient-to-b from-yellow-100 via-yellow-200 to-yellow-100 text-red-900">
      <div className="w-full max-w-md mx-auto rounded-2xl shadow-xl p-8 bg-white/80 border-2 border-yellow-600">
        <h1 className="text-4xl font-extrabold mb-8 tracking-wide drop-shadow-md font-[Cinzel]">
          Đăng ký Triều đình
        </h1>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-red-800">Họ tên</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-red-800">Nhập lại mật khẩu</label>
            <input
              type="password"
              className="mt-1 w-full border-2 border-yellow-600 rounded-md h-10 px-3 focus:ring-2 focus:ring-yellow-500"
            ...
            {pwd && !errors.confirmPassword && (
              <p className="mt-1 text-xs text-red-700">Mẹo: dùng mật khẩu ≥ 6 ký tự</p>
            )}
          </div>
          ...
          {serverMsg && <p className="text-sm mt-2 text-center text-red-700">{serverMsg}</p>}
        </form>
      </div>

      {/* Hoa văn rồng phượng dưới cùng */}
